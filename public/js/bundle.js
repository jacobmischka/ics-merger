...
var require$$0$1 = ( moment$2 && moment$2['default'] ) || moment$2;

var fullcalendar = createCommonjsModule(function (module, exports) {
/*!
 * FullCalendar v3.0.1
...
(function(factory) {
	if (typeof define === 'function' && define.amd) {
		...
		module.exports = factory(jquery, require$$0$1);
	}
	else {
...
Class.extend = function() {
	...
	for (i = 0; i < len; i++) {
		members = arguments$1[i];
		if (i < len - 1) { // not the last argument?
			mixIntoClass(this$1, members);
		}
	}
			...
			if (typeof arg === 'object') { // given dictionary of callbacks
				for (var eventName in arg) {
					if (arg.hasOwnProperty(eventName)) {
						this$1.listenTo(other, eventName, arg[eventName]);
					}
				}
			}
	...
	renderFillSegEls: function(type, segs) {
		...
		if (segs.length) {
			...
			for (i = 0; i < segs.length; i++) {
				html += this$1.fillSegHtml(type, segs[i]);
			}
...
Grid.mixin({
	...
	renderFgSegEls: function(segs, disableResizing) {
		...
		if (segs.length) { // don't build an empty html string
			...
			for (i = 0; i < segs.length; i++) {
				html += this$1.fgSegHtml(segs[i], disableResizing);
			}
	...
	eventRangeToSegs: function(range, event, segSliceFunc) {
		...
		for (i = 0; i < spans.length; i++) {
			segs.push.apply(segs, // append to
				this$1.eventSpanToSegs(spans[i], event, segSliceFunc));
		}
...
var DayTableMixin = FC.DayTableMixin = {
	...
	renderHeadDateCellsHtml: function() {
		...
		for (col = 0; col < this.colCnt; col++) {
			date = this$1.getCellDate(0, col);
			htmls.push(this$1.renderHeadDateCellHtml(date));
		}
	...
	renderBgCellsHtml: function(row) {
		...
		for (col = 0; col < this.colCnt; col++) {
			date = this$1.getCellDate(row, col);
			htmls.push(this$1.renderBgCellHtml(date));
		}
...
var DayGrid = FC.DayGrid = Grid.extend(DayTableMixin, {
	...
	renderDates: function(isRigid) {
		...
		for (row = 0; row < rowCnt; row++) {
			html += this$1.renderDayRowHtml(row, isRigid);
		}
		this.el.html(html);
		...
		for (row = 0; row < rowCnt; row++) {
			for (col = 0; col < colCnt; col++) {
				view.trigger(
					'dayRender',
					null,
					this$1.getCellDate(row, col),
					this$1.getCellEl(row, col)
				);
			}
		}
	},
	...
	renderNumberCellsHtml: function(row) {
		...
		for (col = 0; col < this.colCnt; col++) {
			date = this$1.getCellDate(row, col);
			htmls.push(this$1.renderNumberCellHtml(date));
		}
	...
	spanToSegs: function(span) {
		...
		for (i = 0; i < segs.length; i++) {
			seg = segs[i];
			if (this$1.isRTL) {
				seg.leftCol = this$1.daysPerRow - 1 - seg.lastRowDayIndex;
				seg.rightCol = this$1.daysPerRow - 1 - seg.firstRowDayIndex;
			}
			else {
	...
	renderFill: function(type, segs, className) {
		...
		for (i = 0; i < segs.length; i++) {
			seg = segs[i];
			skeletonEl = this$1.renderFillRow(type, seg, className);
			this$1.rowEls.eq(seg.row).append(skeletonEl);
...
DayGrid.mixin({
	...
	renderSegRows: function(segs) {
		...
		for (row = 0; row < segRows.length; row++) {
			rowStructs.push(
				this$1.renderSegRow(row, segRows[row])
			);
		}
...
DayGrid.mixin({
	...
	limitRows: function(levelLimit) {
		...
		for (row = 0; row < rowStructs.length; row++) {
			...
			if (!levelLimit) {
				...
				rowLevelLimit = this$1.computeRowLevelLimit(row);
			}

			if (rowLevelLimit !== false) {
				this$1.limitRow(row, rowLevelLimit);
			}
		}
	},
	...
	limitRow: function(row, levelLimit) {
		...
		if (levelLimit && levelLimit < rowStruct.segLevels.length) { // is it actually over the limit?
			...
			for (i = 0; i < levelSegs.length; i++) {
				...
				totalSegsBelow = 0;
				while (col <= seg.rightCol) {
					segsBelow = this$1.getCellSegs(row, col, levelLimit);
				...
				if (totalSegsBelow) { // do we need to replace this segment with one or many "more" links?
					...
					for (j = 0; j < colSegsBelow.length; j++) {
						...
						moreLink = this$1.renderMoreLink(
							row,
							seg.leftCol + j,
	...
	renderSegPopoverContent: function(row, col, segs) {
		...
		for (i = 0; i < segs.length; i++) {
			...
			this$1.prepareHits();
			segs[i].hit = this$1.getCellHit(row, col);
			this$1.releaseHits();
...
var TimeGrid = FC.TimeGrid = Grid.extend(DayTableMixin, {
	...
	renderSlatRowHtml: function() {
		...
		while (slotTime < this.maxTime) {
			slotDate = this$1.start.clone().time(slotTime);
			isLabeled = isInt(divideDurationByDuration(slotTime, this$1.labelInterval));

			axisHtml =
				'<td class="fc-axis fc-time ' + view.widgetContentClass + '" ' + view.axisStyleAttr() + '>' +
					(isLabeled ?
						'<span>' + // for matchCellWidths
							htmlEscape(slotDate.format(this$1.labelFormat)) +
						'</span>' :
						''
						) +
				'</td>';
			...
			slotTime.add(this$1.slotDuration);
		}
	...
	spanToSegs: function(span) {
		...
		for (i = 0; i < segs.length; i++) {
			if (this$1.isRTL) {
				segs[i].col = this$1.daysPerRow - 1 - segs[i].dayIndex;
			}
			else {
	...
	sliceRangeByTimes: function(range) {
		...
		for (dayIndex = 0; dayIndex < this.daysPerRow; dayIndex++) {
			dayDate = this$1.dayDates[dayIndex].clone(); // TODO: better API for this?
			dayRange = {
				start: dayDate.clone().time(this$1.minTime),
				end: dayDate.clone().time(this$1.maxTime)
			};
	...
	renderNowIndicator: function(date) {
		...
		for (i = 0; i < segs.length; i++) {
			nodes.push($('<div class="fc-now-indicator fc-now-indicator-line"></div>')
				.css('top', top)
				.appendTo(this$1.colContainerEls.eq(segs[i].col))[0]);
		}
...
TimeGrid.mixin({
	...
	renderFgSegsIntoContainers: function(segs, containerEls) {
		...
		for (col = 0; col < this.colCnt; col++) {
			this$1.updateFgSegCoords(segsByCol[col]);
		}
	...
	computeSegVerticals: function(segs) {
		...
		for (i = 0; i < segs.length; i++) {
			seg = segs[i];
			seg.top = this$1.computeDateTop(seg.start, seg.start);
			seg.bottom = this$1.computeDateTop(seg.end, seg.start);
		}
	},
	...
	assignSegVerticals: function(segs) {
		...
		for (i = 0; i < segs.length; i++) {
			seg = segs[i];
			seg.el.css(this$1.generateSegVerticalCss(seg));
		}
	},
	...
	computeFgSegHorizontals: function(segs) {
		...
		if ((level0 = levels[0])) {
			...
			for (i = 0; i < level0.length; i++) {
				this$1.computeFgSegForwardBack(level0[i], 0, 0);
			}
		}
	},
	...
	computeFgSegForwardBack: function(seg, seriesBackwardPressure, seriesBackwardCoord) {
		...
		if (seg.forwardCoord === undefined) { // not already computed
			...
			for (i=0; i<forwardSegs.length; i++) {
				this$1.computeFgSegForwardBack(forwardSegs[i], 0, seg.forwardCoord);
			}
		}
	},
	...
	assignFgSegHorizontals: function(segs) {
		...
		for (i = 0; i < segs.length; i++) {
			seg = segs[i];
			seg.el.css(this$1.generateFgSegHorizontalCss(seg));
	...
	getUnitViewSpec: function(unit) {
		...
		if ($.inArray(unit, intervalUnits) != -1) {
			...
			for (i = 0; i < viewTypes.length; i++) {
				spec = this$1.getViewSpec(viewTypes[i]);
				if (spec) {
...
Calendar.mixin({
	...
	bindOptions: function(optionNames, handlerFunc) {
		...
		for (i = 0; i < optionNames.length; i++) {
			this$1.registerOptionHandlerObj(optionNames[i], handlerObj);
		}
	...
	triggerOptionHandlers: function(optionName) {
		...
		for (i = 0; i < handlerObjs.length; i++) {
			this$1.triggerOptionHandlerObj(handlerObjs[i]);
		}
	},
	...
	triggerOptionHandlerObj: function(handlerObj) {
		...
		for (i = 0; i < optionNames.length; i++) {
			optionValues.push(this$1.options[optionNames[i]]);
		}
...
Calendar.prototype.isSpanAllowed = function(span, constraint, overlap, event) {
	...
	if (constraint != null) {
		...
		if (constraintEvents) { // not invalid
			...
			for (i = 0; i < constraintEvents.length; i++) {
				if (this$1.spanContainsSpan(constraintEvents[i], span)) {
					anyContainment = true;
					break;
				}
			}
	...
	for (i = 0; i < peerEvents.length; i++)  {
		...
		if (this$1.eventIntersectsRange(peerEvent, span)) {
...
Calendar.prototype.expandBusinessHourEvents = function(wholeDay, inputs, ignoreNoDow) {
	...
	for (i = 0; i < inputs.length; i++) {
		...
		events.push.apply(events, // append
			this$1.expandEvent(
				this$1.buildEventFromInput(input),
				view.start,
				view.end
			)
		);
	...
	spanToSegs: function(span) {
		...
		while (dayStart < view.end) {
			...
			if (
				...
				span.end < dayStart.clone().add(this$1.view.nextDayThreshold)
			) {
	...
	renderSegList: function(allSegs) {
		...
		for (dayIndex = 0; dayIndex < segsByDay.length; dayIndex++) {
			...
			if (daySegs) { // sparse array, so might be undefined
				...
				// append a day header
				tbodyEl.append(this$1.dayHeaderHtml(
					this$1.view.start.clone().add(dayIndex, 'days')
				));

				this$1.sortEventSegs(daySegs);
...
var gcal = createCommonjsModule(function (module, exports) {
/*!
...
fetch('/.env.json')
	.then(function (response) {
		...
		var apiKey = dotenv.GOOGLE_CALENDAR_API_KEY;
		var calendar = dotenv.calendars[calendarId];
		...
		if(calendar){
			...
			jquery('#calendar').fullCalendar({
				...
				eventSources: googleCalendarIds.map(function (id) {
					return {
						googleCalendarId: id
					};
				})
